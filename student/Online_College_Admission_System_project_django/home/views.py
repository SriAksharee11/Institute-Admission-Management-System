...
def contact(request):
    if request.method == "POST":
        ...
        document = Contact.objects.create(name=name,father=father,mother=mother,email=email, phone=phone, phone1=phone1,Address1=Address1, Address2=Address2, SchoolName=SchoolName,file=file,Aadhar=Aadhar, desc=desc,sslc=sslc)#JeeAppNo=JeeAppNo)
        document.save()
        messages.success(request, 'Your message has been sent!,Go To Studentinfo')
    return render(request, "contact.html")
...
def student(request):
    if request.method == "POST":
        ...
        students = Student.objects.order_by('-hscmark')
        ...
        if int(hscmark)>500:
            ...
            send_mail(
                    ...
                    'dear applicant your mark eligible for this admission',
        ...
        document=Student.objects.create(Username=Username,email=email,hscschool=hscschool,hscmark=hscmark,)
        document.save()
    ...
    #return render(request, 'email.html', {'students': students})  
...
def course(request):
    if request.method == "POST":
        ...
        #ugmark=request.POST.get('ugmark')
        category = request.POST.get('category')
        selected_course = request.POST.get('ugcourse' if category == 'ug' else 'pgcourse')
        if int(hscpercentage)>85:
            print('your mark eligible for admission')
            #sub='Kongu enginieering college'
            #msg='your application was successfully sumbited wait for the result'
            send_mail(
        ...
        document=Course.objects.create(studentname=studentname,hscpercentage=hscpercentage,category=category,selected_course=selected_course,email=email)#ugmark=ugmark)#pgcourse=pgcourse)
        document.save()
